...
def train_loop(args, loop_num: int, vis=True, start_from=0):
    ...
    for loop in range(start_from, loop_num):
        ...
        if loop == 0:
            ...
            args.output_dir_per_loop = os.path.join(
                output_dir_base, args.character_name, str(loop - 1)
            )

            # load model from the output dir in PREVIOUS loop
            pipe = load_trained_pipeline(
                model_path=args.output_dir_per_loop,
                load_lora=True,
                lora_path=os.path.join(
                    args.output_dir_per_loop,
        ...
        args.output_dir_per_loop = os.path.join(
            output_dir_base, args.character_name, str(loop)
        )

        # set up the training data folder used in training, overwrite and recreate
        args.train_data_dir_per_loop = os.path.join(
            train_data_dir_base, args.character_name, str(loop)
        )
        ...
        for n_img in range(args.num_of_generated_img):
            print(
                f"[Loop [{loop}/{loop_num-1}], generating image {n_img}/{args.num_of_generated_img - 1}"
            )
            ...
            if loop == 0 and os.path.exists(os.path.join(tmp_folder, f"{n_img}.png")):
                image = Image.open(os.path.join(tmp_folder, f"{n_img}.png")).convert(
                    "RGB"
                )
            else:
                image = generate_images(
                    pipe, prompt=args.inference_prompt, infer_steps=args.infer_steps
                )
        ...
        if loop == 0:
            init_dist = np.mean(
                cdist(embeddings, embeddings, "euclidean")
            )  # calculates the average distance between all the images
        else:
            ...
            if pairwise_distances < init_dist * args.convergence_scale:
                ...
                print()
                return (
                    os.path.join(output_dir_base, args.character_name, str(loop - 1)),
                )
            else:
                print()
                print(
                ...
                print(
                    f"Target distance: {init_dist}, current pairwise distance: {pairwise_distances}."
                )
                print(
                    "###########################################################################"
                )
                ...
                print()
        # clustering
        centers, labels, elements, images = kmeans_clustering(
            args, embeddings, images=images
        )
        ...
        center_norms = np.linalg.norm(
            centers[labels] - elements, axis=-1, keepdims=True
        ...
        for label_id in range(len(np.unique(labels))):
            cohesions[label_id] = sum(center_norms[labels == label_id]) / sum(
                labels == label_id
            )
        ...
        for sample_id, sample in enumerate(images):
            if sample_id in idx:
                sample.save(
                    os.path.join(args.train_data_dir_per_loop, f"{sample_id}.png")
                )
...
def kmeans_clustering(args, data_points, images=None):
    ...
    selected_clusters = [
        cluster for cluster, count in cluster_counts.items() if count > args.dmin_c
    ]
    ...
    selected_elements = np.array(
        [data_points[i] for i, label in enumerate(labels) if label in selected_clusters]
    )
    if images:
        selected_images = [
            images[i] for i, label in enumerate(labels) if label in selected_clusters
        ]
    else:
...
def kmeans_2D_visualize(args, centers, data, labels, loop_num):
    ...
    for i in range(args.kmeans_center):
        ...
        plt.scatter(
            cluster_points[:, 0], cluster_points[:, 1], label=f"Cluster {i + 1}", s=100
        )
...
def generate_images(
    pipe: StableDiffusionXLPipeline, prompt: str, infer_steps, guidance_scale=7.5
):
    """
    ...
    """
    image = pipe(
        prompt, num_inference_steps=infer_steps, guidance_scale=guidance_scale
    ).images[0]
    return image
